...
PresenterAI sends data only after an explicit typed request or acknowledged toggle-listening operation. Responses use `store:false`; the application creates no OpenAI Conversation and stores no cloud meeting history. Bounded WAV data is deleted locally when transcription reaches a terminal state, before retrieval or response generation.
...
| Gate | Result |
|---|---|
| TypeScript and full Vitest suite | Current tree passed; 44 files, 323/323 tests |
...
| .NET helper tests and WASAPI smoke | 30/30; protocol v2, selected render endpoint, valid 16 kHz mono WAV |
| Playwright Electron suite | Passed 7/7, including two sequential toggle operations in one app session |
...
- Current TypeScript/Vitest/.NET/Playwright: passed; 323/323, 30/30, and 7/7 respectively.
...
- Current offline/source gate: audit, 323/323 Vitest, 30/30 .NET, M4 50/50, M7 50/50, M6 zero-network preflight, helper smoke, and 7/7 Playwright passed on 2026-07-18. The current unsigned installer builds locally and its packaged FTS probe passed through the controlled NSIS path. Enterprise Code Integrity then blocked the same new unsigned application hash during the packaged-helper and installer-launch probes, which remain fail-closed without disabling or bypassing Windows security. Clean Windows CI remains authoritative for a future delivery of these uncommitted changes.
