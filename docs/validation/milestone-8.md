...
- Current delivery target: `PresenterAI-0.2.0-beta.4`. Installer lifecycle validation retains the strict current-build hooks, redacted lifecycle and Code Integrity diagnostics, and a SHA-256 manifest.
- Historical beta.3 local source gate: audit with 0 vulnerabilities, 378/378 Vitest, 33/33 .NET tests, 10/10 Playwright, M4 50/50, M7 50/50, M6 zero-network preflight, production build, NSIS packaging, packaged FTS/helper probes, two real WASAPI smoke cycles, and clean installer smoke completed on 2026-07-23. Smart App Control reported `enforced` and the helper remained unsigned, but the controlled helper spawn returned protocol-v2 `ready` in that run. Beta.4 retains these gates and adds the current clear-glass/compact-Copilot checks in `clear-glass-compact-copilot.md`; its pull-request and post-merge Windows workflows must be green before distribution. These smoke results do not replace the outstanding physical/Meet/manual gates.
