...
| The overlay now uses a wider hybrid Acrylic/WebGL liquid-glass presentation. | The prior 560px shell compressed presenter content, while earlier native material produced rectangular bleed beneath CSS corners. A versioned bounds migration expands legacy windows; Acrylic is requested only on supported Windows 11 builds, a native rounded region clips the backdrop, and a pointer-free shader supplies bounded blue-violet caustics with reduced-motion/transparency and forced-colors fallbacks. This is presentation-layer hardening, not an architectural change. |
