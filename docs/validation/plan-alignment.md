...
| M3 | The original structured, no-invention, budget, and live Luna/Terra gate is accepted. The beta.4 natural spoken-delivery revision is offline-green but awaits separately authorized live revalidation. |
...
| The beta.4 overlay replaces beta.3's hybrid Acrylic/native-shape experiment with clear shader-first glass. | Target-machine review showed that full-window Acrylic was too frosted and Electron exposed no useful tint/blur controls, while the experimental polygonal `setShape()` region produced visibly stepped corners. The current transparent BrowserWindow relies on Windows' rounded-corner preference plus a consistent 24px renderer clip; a pointer-free shader supplies localized blue-violet caustics with reduced-motion/transparency and forced-colors fallbacks. This is presentation-layer hardening, not an architectural change. |
| Code is the default answer style and Presenter is a one-request override. | The user explicitly prioritized coding tasks after testing the generic presenter default. Developer output now has its own strict schema and local discriminator; the accepted presenter provider prompt/schema remains unchanged, grounding still applies, and no code execution or tools were added. |
| Presenter responses now include natural spoken-delivery guidance. | The user found the prior presenter output too mechanical for live use. The new revision changes delivery instructions only: the schema, grounding invariants, category precedence, models, token limits, `store:false`, and one-request flow remain fixed. The accepted M3 run is retained as historical evidence, while the new prompt fingerprint is explicitly offline-green/live-pending. |
