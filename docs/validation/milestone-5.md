...
Latest beta.2 local regression run: 2026-07-22 on Windows 11 build 26200, Electron 43.1.0. It passed 353 Vitest tests in 50 files, 33/33 .NET tests, 9/9 Playwright Electron tests, an audit with zero vulnerabilities, and the accepted 50/50 M4 retrieval corpus. A strict two-cycle helper smoke produced valid 16 kHz mono captures through the selected render endpoint, and packaged FTS/helper probes passed after reputation evaluation. GitHub run 29908576569 had no active render endpoint; this is recorded as unavailable rather than a WASAPI pass. CI therefore runs the published helper through two protocol, conversion, finalization, and cleanup cycles with a dual-gated deterministic test backend whose handshake omits the WASAPI feature and explicitly reports `wasapiCaptureValidated: false`. Code Integrity has also blocked unsigned local payloads, so the isolated local results do not replace clean CI or physical Meet evidence.
...
|---|---|---|---|
| M5-AUTO-01 | Audit, typecheck, Vitest, helper tests, Playwright, M4 regression, production build | Pending clean CI | Local audit: 0 vulnerabilities; Vitest: 50 files / 353 tests; .NET: 33/33 in the latest isolated run; Playwright: 9/9; M4: 50/50; typecheck and production build pass. Prior Code Integrity blocks remain recorded, and zero discovered tests is a hard failure. |
