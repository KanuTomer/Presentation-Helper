...
function App(): React.JSX.Element {
  ...
  return <main className={`shell ${status.listening ? 'is-listening' : ''}`}>
    ...
    <section className="content no-drag" tabIndex={0} aria-label={`${view} content`} onKeyDown={handleContentScrollKey}>
      ...
      }} answerFormat={answerFormat} setAnswerFormat={setAnswerFormat} input={input} ask={ask} response={response} aiError={aiError ?? status.operationError} openSettings={() => setView('settings')} openPrivacy={() => setView('privacy')} hasKey={apiKeyStatus.configured} status={status} onAudioError={showAiError} />}
  ...
  return <div className="stack">
    ...
    <div className="question-box">
      ...
      <div className="composer-toolbar">
        ...
        <div className="actions"><button className="primary" disabled={!canSubmit} title={busy ? 'Another operation is active.' : props.transcriptDraft?.conflict ? 'Resolve the pending transcript first.' : undefined} onClick={() => void props.ask()}>Ask AI <kbd>Ctrl↵</kbd></button>
...
function Settings({ settings, status, recordingMs, apiKeyStatus, onChange }: { settings: AppSettings; status: AppStatus; recordingMs: number; apiKeyStatus: ApiKeyStatus; onChange(): Promise<void> }) {
  ...
  return <div className="stack"><h2>Settings</h2>
    ...
    <fieldset><legend>Overlay</legend><label>Glass tint <input type="range" min="0.18" max="0.78" step="0.01" value={settings.glassTint} onChange={(e) => void update({ glassTint: Number(e.target.value) })} /></label><p className="muted">Tint changes the glass surface only. Text and code remain fully opaque; click-through is controlled separately.</p><label className="toggle"><input type="checkbox" checked={settings.clickThrough} onChange={(e) => void update({ clickThrough: e.target.checked })} /> Click-through mode</label></fieldset>
