...
export function ResponseCard({ response, onCopyCode }: { response: AssistantResponse; onCopyCode?: CopyCodeHandler }): React.JSX.Element {
  ...
  if (response.responseStyle === 'developer') {
    return <article className="response-card developer-response">
      ...
      <p className="developer-summary">{response.summary}</p>
      ...
      <ul>{response.implementationNotes.map((note, index) => <li key={`${index}:${note}`}>{note}</li>)}</ul>
      {(response.caveats?.length ?? 0) > 0 && <>
        <h3>CAVEATS</h3>
        <ul>{response.caveats?.map((caveat, index) => <li key={`${index}:${caveat}`}>{caveat}</li>)}</ul>
      </>}
      <ResponseEvidence response={response} />
    </article>
  }
...
function ResponseEvidence({ response }: { response: AssistantResponse }): React.JSX.Element {
  return <>
