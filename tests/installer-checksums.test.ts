...
describe('installer checksum manifest', () => {
  it('writes a SHA-256 entry for only the package-version installer', async () => {
    ...
    await writeFile(join(directory, 'PresenterAI-0.2.0-beta.1-setup.exe'), 'current')
    ...
    expect(entries.map((entry) => entry.filename)).toEqual(['PresenterAI-0.2.0-beta.1-setup.exe'])
    expect(manifest).toBe(`${digest('current')}  PresenterAI-0.2.0-beta.1-setup.exe\n`)
  })
